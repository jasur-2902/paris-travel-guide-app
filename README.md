...
### Activity Management
- **48 pre-loaded Paris activities** including museums, landmarks, parks, and restaurants
- **Sortable table** with columns for date, type, metro station, hours, price, and transit time
- **Filter & search** to instantly find activities by name or type
- **Completion tracking** to check off visited places with animated checkmarks
- **Hide/unhide** activities you're not interested in
- **Add custom activities** with geocoded address search for any location in France
- **Notes** for adding personal notes to any activity

### Itinerary Planner
- **Drag-and-drop** day-by-day itinerary with @dnd-kit
- **Split view** with itinerary panel + interactive map side by side
- **Day route lines** on the map connecting planned activities
- **Closed-day warnings** that alert when an activity is likely closed on your planned date
...
- **Day route visualization** with dashed lines connecting same-day activities
...
### Budget Tracker
- **Collapsible budget bar** showing spent, budget, and remaining at a glance
- **Expandable details** with planned costs, total breakdown, and editable budget target
...
### Transit Times
- **Hybrid calculation** using OSRM walking times + metro estimation
- **Set your base address** via search or geolocation
- **Per-activity transit times** updated automatically
- **Day trip detection** so locations outside Paris keep default times

### Dark Mode
- **Full dark theme** with dark map tiles (CSS filter inversion)
- **Dark Leaflet popups and controls** for a seamless dark experience
- **Persisted** to localStorage, remembers your preference

![Dark Mode Table View](screenshots/dark-mode-main.png)

### Simple & Full Views
- **Simple mode** shows only essential columns (date, activity, type, price)
- **Full mode** shows all columns plus weather pill, budget tracker, and hidden items toggle
- **Smart defaults** where new users start in Simple mode; returning users get their saved preference

![Dark Mode Simple View](screenshots/dark-mode-simple-mode.png)

### Multi-Language Support
- **English, Spanish, and French** with 170+ translation keys
...
- **Print stylesheet** for clean printable layouts
- **PWA / offline support**: installable as a standalone app with service worker caching
- **Mobile responsive** with card layout on small screens, floating action button, touch-optimized
...
- **Click-outside dismiss** uses `useRef` + `useEffect` with `mousedown` listener (settings, weather pill, kebab menus, address dropdown)
- **Hybrid transit estimation** combines OSRM walking time + metro heuristic based on distance thresholds
- **Progressive disclosure** via Simple/Full view toggle following Hick's Law
- **Spotlight tutorial** uses CSS `box-shadow: 0 0 0 9999px` overlay with positioned tooltip
- **i18n via React Context** where `useLanguage()` hook provides `t()` function with parameter interpolation
- **Responsive breakpoints** with desktop table at `md+`, card layout below; responsive grid for weather
